...
def create_order(internal_order: InternalOrder):
    ...
    # 通过异步分发任务（本地用 Celery，Lambda 用 SQS）
    import os
    if os.getenv('AWS_LAMBDA_FUNCTION_NAME'):  # Lambda 环境自动有此变量
        import boto3, json
        boto3.client('sqs', region_name=os.getenv('AWS_REGION', 'us-east-2')).send_message(
            QueueUrl=os.getenv('SQS_QUEUE_URL'),
            MessageBody=json.dumps({'order_id': str(order.id)})
        )
    else:
        from careplan.tasks import generate_care_plan
        generate_care_plan.delay(str(order.id))  # 本地 Django 不动
